 ...
 */	
	public boolean isTriangle(Halfedge h) {
		throw new Error("To be completed");
	}
 ...
 */	    
    public boolean isValid(boolean borders) {
        ...
        for(int i=0; i<this.halfedges.size(); i++) {
        	Halfedge pedge=this.halfedges.get(i);
        ...
        for(int i=0; i<this.vertices.size(); i++){
        	Vertex pvertex=this.vertices.get(i);
     ...
     */
    public void createCenterVertex(Face f){
    	throw new Error("To be completed");
    }
 ...
 */	       
    ...
    public Halfedge makeHole(Halfedge h){
    ...
    public Halfedge fillHole(Halfedge h){
 ...
 */	        
    public Halfedge addTriangleToBorder(Halfedge h, Point_3 point) {
    	if(h.face!=null) throw new Error("no border edge");
    	System.out.println("adding triangle to "+h);
    	
    	Face newFace=new Face();
    	Vertex newVertex=new Vertex(point);
    	Halfedge hPrev=new Halfedge();
    	Halfedge hNext=new Halfedge();
    	Halfedge hPrevOpp=new Halfedge();
    	Halfedge hNextOpp=new Halfedge();
 ...
 */	        
 	public Halfedge makeTriangle(Point_3 p1, Point_3 p2, Point_3 p3) {    	
    	Face newFace=new Face();
    	Vertex newVertex1=new Vertex(p1);
    	Vertex newVertex2=new Vertex(p2);
    	Vertex newVertex3=new Vertex(p3);
    	Halfedge e1=new Halfedge();
    	Halfedge e2=new Halfedge();
    	Halfedge e3=new Halfedge();
    	Halfedge e1Opp=new Halfedge();
    	Halfedge e2Opp=new Halfedge();
    	Halfedge e3Opp=new Halfedge();
 ...
 */	    
    public Halfedge splitFacet(Halfedge h, Halfedge g) {
		...
		Halfedge newDiagonal=new Halfedge();
		Halfedge oppositeNewDiagonal=new Halfedge();
		Face newFace=new Face();
     ...
     */	    
        public Halfedge splitEdge(Halfedge h, Point_3 point) {
        	if(h==null) {
        		throw new Error("splitFacet: null pointer");
        	}
        	...
        	Halfedge hNewLeft=new Halfedge();
    		Halfedge hNewRight=new Halfedge();
    		Vertex newVertex=new Vertex(point);
    		...
    		Vertex u=h.getOpposite().getVertex();
