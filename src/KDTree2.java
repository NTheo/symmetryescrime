...
public class KDTree2<T> {
	...
	private class NodeKD {
			...
			if (isLeaf) {
				...
				if (data[sliceDimension] > slice) {
					...
					if (left.current == 0)
						return;
					if (!heap.isFull()
							|| regionDistSq(data, left.minBounds,
									left.maxBounds) < heap.getMaxKey()) {
					...
					if (right.current == 0)
						return;
					if (!heap.isFull()
							|| regionDistSq(data, right.minBounds,
									right.maxBounds) < heap.getMaxKey()) {
						right.nearest(heap, data);
					}
				}
			}
		...
		public boolean intersects(double[] up0, double[] low0, double[] up1,
				double[] low1) {
	...
	private static final double regionDistSq(double[] point, double[] min,
			double[] max) {
		double d = 0;
		double q = 0;
		for (int i = 0; i < point.length; ++i) {
