...
public class LoadMesh {
	...
	public Polyhedron_3 createPolyhedron(Point_3[] points, int[] faceDegrees, int[][] faces, int sizeHalfedges)
    {   
    	int i,j;
    	Polyhedron_3 result=new Polyhedron_3(points.length,sizeHalfedges,faceDegrees.length);
        
        //System.out.println("Creating a new polyhedron");

        for(i=0;i<points.length;i++){
            result.vertices.add(new Vertex(points[i]));
        }
        ...
        for(i=0;i<faceDegrees.length;i++) {         
            _face=new Face(); // creating a face
            result.facets.add(_face);
            
            Halfedge _edge, _prev=null, _begin=null;
        ...
        for(i=0;i<result.halfedges.size();i++) {
        	Halfedge pedge=(Halfedge)result.halfedges.get(i);
	 ...
	 */
	public Polyhedron_3 createTriangleMesh(Point_3[] points, int[] faceDegrees, int[][] faces, int sizeHalfedges)
    {   
    	...
    	HashMap<HalfedgePair, Halfedge> insertedHalfedges=new HashMap(sizeHalfedges);
        ...
        for(i=0;i<faceDegrees.length;i++) {         
            ...
            if(insertedHalfedges.containsKey(pair0)==false) {
            	...
            	Halfedge eOpposite=insertedHalfedges.get(pair0);
            ...
            if(insertedHalfedges.containsKey(pair1)==false) {
            	...
            	//System.out.println("edge already inserted");
            	Halfedge eOpposite=insertedHalfedges.get(pair1);
            ...
            if(insertedHalfedges.containsKey(pair2)==false) {
            	...
            	//System.out.println("edge already inserted");
            	Halfedge eOpposite=insertedHalfedges.get(pair2);
